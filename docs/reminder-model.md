...
Recurring series require the original calendar identifier and use a shared external identifier when both events provide one. Conflicting known identifiers never fall through to title matching; only absent series identifiers permit the existing title/time fallback within that calendar. Separately-created events such as Meetup imports are handled through fuzzy matching.

A saved occurrence combines its calendar and series identity with EventKit’s original occurrence date, keeping current start and end separately. Exact native lookup materializes candidates only in that calendar, in bounded three-day windows around the saved current and original dates; identifier lookups contribute candidates but cannot choose a series head. A direct candidate can prove a far-moved instance through its original date. Older snapshots without an original date use a unique compatible identifier plus their exact saved start, with an exact title/start fallback only when identifiers cannot establish the occurrence. Missing or ambiguous identity never selects another event. An ambiguous pin stays unconsumed until identity can be established; a missing pin retains its existing end-based retirement behavior. Ambiguous occurrences cannot establish new pins or activities: one-time matching waits when an earlier uncertain occurrence could be its target, while every-match rules may retain independently proven occurrences and report incomplete matching.
