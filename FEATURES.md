...
- Stores notes and audio locally for offline use and includes them in device backups. Transient stage failures become eligible for retry after 1, 5, and 15 minutes, then wait for manual Retry; unavailable services and unreadable audio wait for manual Retry immediately.
- Saving, Retry, Reprocess, and saved reminder feedback request continued processing when started in the foreground, with native system progress while transcription, analysis, and reminder preparation continue after switching apps or locking the screen. Interrupted work also requests an automatic background retry when iOS grants time, and resumes on reopening; force-closing the app stops current processing, and background completion timing depends on iOS. Canceling system progress preserves the audio and completed stages.
